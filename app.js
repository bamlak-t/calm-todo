...
const App = (function () {
  ...
  function initSupabase() {
    if (window.supabase && supabaseUrl && supabaseKey) {
      try {
        ...
        console.warn(
          "Supabase init error, falling back to local storage:",
          err,
        );
  ...
  async function fetchSessionsFromDB() {
    ...
    try {
      ...
      if (dbSessions && dbSessions.length > 0) {
        // Fetch subevents
        const { data: dbEvents } = await supabaseClient
          .from("session_events")
          .select("*");
  ...
  async function syncAllToDB() {
    ...
    try {
      ...
      for (const s of sessions) {
        const isNewSession =
          typeof s.id === "string" && s.id.startsWith("s-") && s.id.length < 30;
        if (isNewSession) {
          newSessions.push(s);
        } else {
      ...
      if (existingSessionsPayload.length > 0) {
        const { error } = await supabaseClient
          .from("todo_sessions")
          .upsert(existingSessionsPayload);
        if (error) throw error;
      }

      // 2. Insert new sessions one-by-one to get their UUIDs
      for (const s of newSessions) {
        const { data: upsertedSession, error } = await supabaseClient
          .from("todo_sessions")
          .upsert({
            title: s.title,
            rank: s.rank,
      ...
      if (existingEventsPayload.length > 0) {
        const { error } = await supabaseClient
          .from("session_events")
          .upsert(existingEventsPayload);
        if (error) throw error;
      }

      // 5. Insert new events one-by-one to get their UUIDs
      for (const ne of newEvents) {
        const { data: upsertedEv, error } = await supabaseClient
          .from("session_events")
          .upsert({
  ...
  function renderMonthView(grid, year, month) {
    ...
    const nextDaysNeeded =
      (currentCellsCount > 35 ? 42 : 35) - currentCellsCount;
  ...
  function createDayCell(dayNumber, isOtherMonth, dateStr, isToday = false) {
    ...
    if (dateStr && !isOtherMonth) {
      ...
      allocatedSessions.forEach((s) => {
        ...
        if (s.events) {
          const eventsOnDay = s.events.filter(
            (sub) => (sub.event_date || s.allocated_date) === dateStr,
          );
          eventsOnDay.forEach((sub) => {
            const subChip = document.createElement("div");
            subChip.className = `event-chip ${sub.completed ? "completed" : ""}`;
            subChip.style.background =
              CATEGORIES[sub.category]?.color || "var(--accent-yellow)";
            const catIcon = CATEGORIES[sub.category]?.icon
  ...
  function setCalendarView(view) {
    calendarView = view;
    document
      .getElementById("view-month-btn")
      .classList.toggle("active", view === "month");
    document
      .getElementById("view-week-btn")
  ...
  function renderSessions() {
    ...
    const pendingCount = sessions.filter((s) => {
      const isCompleted =
        s.completed ||
        (s.events && s.events.length > 0 && s.events.every((e) => e.completed));
      return !isCompleted;
    }).length;
    ...
    if (searchQuery) {
      filteredSessions = sessions.filter(
        (s) =>
          ...
          (s.events &&
            s.events.some((e) => e.title.toLowerCase().includes(searchQuery))),
      );
    }
    ...
    paginatedSessions.forEach((session, index) => {
      ...
      const dateBadge = isInbox
        ? ""
        : session.allocated_date
            ...
            ${(session.events || [])
              .map((e) => {
                ...
                const locationBadge = e.location
                  ? `<a href="${e.location.startsWith("http") ? e.location : "#"}" target="_blank" class="badge" style="background: var(--bg-color); font-size: 0.65rem; border: 1px solid var(--text-muted); text-decoration: none; color: var(--text-muted); cursor: pointer;" title="${escapeHtml(e.location)}">📍 ${e.location.startsWith("http") ? "Map" : escapeHtml(e.location)}</a>`
                  : "";
                const displayTime = e.event_time
            ...
            <div class="pills-row">
              <div class="category-pills" data-selected="general">
                ${Object.entries(CATEGORIES)
                  .map(
                    ([key, cat]) =>
                      `<button type="button" class="cat-pill ${key === "general" ? "active" : ""}" data-cat="${key}" style="--pill-color: ${cat.color}" onclick="App.selectCategoryPill(this)">${cat.name}</button>`,
                  )
                  .join("")}
              </div>
            </div>
  ...
  function promptAllocateDate(sessionId) {
    ...
    const dateStr = prompt(
      `Allocate "${session.title}" to date (YYYY-MM-DD):`,
      session.allocated_date || formatDateIso(new Date()),
    );
    if (dateStr !== null) {
      allocateSessionToDate(sessionId, dateStr);
    }
  ...
  function toggleNewItemType() {
    const type = document.querySelector(
      'input[name="new_item_type"]:checked',
    ).value;
    if (type === "session") {
  ...
  function openNewItemModal() {
    ...
    document.querySelector(
      'input[name="new_item_type"][value="event"]',
    ).checked = true;
  ...
  function saveSessionForm(e) {
    ...
    if (type === "event" && !id) {
      const targetSessionId = document.getElementById(
        "new-event-target-session",
      ).value;
      ...
      const newEvent = {
        ...
        category:
          document.getElementById("new-event-category-pills").dataset
            .selected || "general",
        completed: false,
        event_date:
          document.getElementById("new-event-date").value ||
          targetSession.allocated_date ||
          formatDateIso(new Date()),
        event_time: document.getElementById("new-event-time").value
          ? document.getElementById("new-event-time").value + ":00"
          : "13:00:00",
    ...
    const rank =
      parseInt(document.getElementById("form-session-rank").value) || 1;
    const date = document.getElementById("form-session-date").value;
    const endDate =
      document.getElementById("form-session-end-date").value || date;
  ...
  function openSessionEditor(sessionId) {
    ...
    document.getElementById("form-session-date").value =
      session.allocated_date || "";
    document.getElementById("form-session-end-date").value =
      session.end_date || session.allocated_date || "";
    ...
    const sessionRadio = document.querySelector(
      'input[name="new_item_type"][value="session"]',
    );
  ...
  function handleAddSubEvent(e, sessionId) {
    ...
    const category = pillsContainer
      ? pillsContainer.dataset.selected
      : "general";
      ...
      session.events.push({
        ...
        event_date:
          dateInput && dateInput.value
            ...
            : session.allocated_date || formatDateIso(new Date()),
        event_time:
          timeInput && timeInput.value ? timeInput.value + ":00" : "13:00:00",
  ...
  function editSubEvent(sessionId, eventId) {
    ...
    document.getElementById("edit-subevent-time").value = sub.event_time
      ? sub.event_time.substring(0, 5)
      : "";
    document.getElementById("edit-subevent-location").value =
      sub.location || "";
  ...
  function saveSubEventForm(e) {
    ...
    sub.category =
      document.getElementById("edit-subevent-category").dataset.selected ||
      "general";
    sub.event_date =
      document.getElementById("edit-subevent-date").value ||
      session.allocated_date ||
      null;
    const timeVal = document.getElementById("edit-subevent-time").value;
    sub.event_time = timeVal ? timeVal + ":00" : null;
    sub.location = document
      .getElementById("edit-subevent-location")
      .value.trim();
  ...
  function moveSubEvent(sessionId, eventId) {
    ...
    const optionsHtml = targetSessions
      .map(
        (s, idx) => `
    ...
    // Reset inputs
    document.getElementById("move-create-own-dates").style.display =
      targetSessions.length === 0 ? "flex" : "none";
    document.getElementById("move-create-own-start").value = formatDateIso(
      new Date(),
    );
    document.getElementById("move-create-own-end").value = formatDateIso(
      new Date(),
    );
  ...
  async function saveMoveSubEvent(e) {
    ...
    const targetSessionId = document.querySelector(
      'input[name="move-target"]:checked',
    )?.value;
    ...
    if (targetSessionId === "CREATE_OWN") {
      const startDate =
        document.getElementById("move-create-own-start").value ||
        formatDateIso(new Date());
      const endDate =
        document.getElementById("move-create-own-end").value || startDate;
    ...
    const targetSession = sessions.find(
      (s) => s.id === targetSessionIdResolved,
    );
    if (!targetSession) return;
  ...
  function openDayDetail(dateStr) {
    ...
    allocatedSessions.forEach((s) => {
      ...
      if (s.events && s.events.length > 0) {
        const eventsOnDay = s.events.filter(
          (ev) => (ev.event_date || s.allocated_date) === dateStr,
        );
    ...
    if (allTimelineEvents.length === 0) {
      html =
        '<p style="text-align: center; color: var(--text-muted); padding: 24px 0; font-size: 0.9rem;">Nothing planned for this day.</p>';
    } else {
      html = '<div style="display: flex; flex-direction: column; gap: 2px;">';
      allTimelineEvents.forEach((ev) => {
        const timeLabel = ev.time || "All day";
        const completedStyle = ev.completed
  ...
  function renderExternalCalendarsList() {
    ...
    if (externalCalendars.length === 0) {
      container.innerHTML =
        '<p style="font-size: 0.8rem; color: var(--text-muted);">No external calendars connected yet.</p>';
      return;
    }
    ...
    externalCalendars.forEach((c) => {
      ...
      item.style.cssText =
        "display: flex; justify-content: space-between; align-items: center; background: var(--bg-color); border: 1.5px solid var(--border-color); padding: 8px 12px; border-radius: var(--radius-sm); font-size: 0.85rem;";
      item.innerHTML = `
        <div>
